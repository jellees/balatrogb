...
PNGS = $(call rwildcard,src,*.png)
2BPP = $(PNGS:.png=.2bpp)
2BPP := $(patsubst src/%,%,$(2BPP))

PALETTES = $(call rwildcard,src,*.palette)
PAL_FILES = $(patsubst src/%,%,$(PALETTES:.palette=.pal))
...
assets/%.2bpp: src/assets/%.png
	$(RGBGFX) -m @src/assets/$*.flags -o $@ src/assets/$*.png

assets/%.pal: src/assets/%.palette
	$(RGBGFX) -c hex:$< -p $@
